...
#if IRLOCK == ENABLED
void Copter::update_irlock(void)
{
static uint32_t last_of_update = 0;
if (!irlock.enabled()){
return;}
...
//IRLOCK_FRAME[i].signature, IRLOCK_FRAME[i].center_x, IRLOCK_FRAME[i].center_y, IRLOCK_FRAME[i].width, RLOCK_FRAME[i].height);
//}
//---------------------------------------------------------------------------------------------
}
else
